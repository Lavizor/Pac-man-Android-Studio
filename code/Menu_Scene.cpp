        ...
        if (state == READY)                     // Se descartan los eventos cuando la escena está LOADING
        {
            switch (event.id)
            {
                ...
                case ID(touch-ended):           // El usuario deja de tocar la pantalla
                {
                    ...
                    if (option_at (touch_location) == PLAY&&!helping)
                    {
    ...
    void Menu_Scene::update (float time)
    {
        if (!suspended) if (state == LOADING)
        {
            ...
            if (context)
            {
                ...
                helper = Texture_2D::create (ID(test), context, "help.png");
        ...
        if (!suspended)
        {
            ...
            if (canvas)
            {
                ...
                if (state == READY)
                {
                    ...
                    if(helping){
                        canvas->fill_rectangle({canvas_width/2,canvas_height/2},{1280,720},helper.get());
                    }
                }
            }
        }
